...
function sqlite_api(){
	...
	this.listVisits = function(id, fn){
		if(id){
			db.all("select * from pixel_requests where pixel_id = ?", id, function(err, rows){
				if(!err){
					fn(null, rows)
				}else{
					fn(err)
	...
	this.listAllVisits = function(fn){
		db.all("select * from pixel_requests", function(err, rows){
			if(!err){
				fn(null, rows)
			}else{
				fn(err)
